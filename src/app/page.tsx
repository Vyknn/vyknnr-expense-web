...
import { ROUND_STATUS_BADGE_CLASSES, ROUND_STATUS_LABELS, type RoundStatus } from "@/lib/round-status";
import { getRounds, type RoundListItem } from "./queries";
  ...
  return (
    <div className="mx-auto flex w-full min-w-0 max-w-7xl flex-1 flex-col gap-6 px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
      ...
      <section aria-label="สรุปรอบเบิก-จ่าย" className="grid gap-3 sm:grid-cols-3">
        ...
        <SummaryCard label="รอดำเนินการ" value={`${inProgressRounds.length} รอบ`} detail={inProgressRounds.length > 0 ? "ติดตามและอัปเดตสถานะได้" : "ไม่มีรอบที่รอดำเนินการ"} />
      </section>
      ...
      <RoundGroup
        ...
        canEdit={canEdit}
        emptyMessage="ยังไม่มีรอบที่กำลังดำเนินการ"
      />
      <RoundGroup
        status="completed"
        rounds={completedRounds}
        canEdit={canEdit}
        emptyMessage="ยังไม่มีรอบที่เสร็จสิ้น"
      />
      <RoundGroup
        status="cancelled"
        rounds={cancelledRounds}
        canEdit={canEdit}
        emptyMessage="ยังไม่มีรอบที่ยกเลิก"
      />
    </div>
  );
}
  ...
  return (
    <section className="min-w-0 overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      <div className="flex flex-col gap-3 border-b border-border px-3 py-1.5 sm:flex-row sm:items-center sm:justify-between sm:px-5">
        <div>
          <h2 className="font-semibold">{ROUND_STATUS_LABELS[status]}</h2>
          <p className="mt-0.5 text-sm text-muted">{rounds.length} รอบ</p>
        </div>
      </div>
              ...
              <li key={round.id} className="rounded-xl border border-border bg-card p-4 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <Link href={`/rounds/${round.id}`} className="font-semibold text-foreground hover:text-primary hover:underline">
                      {round.name}
                    </Link>
                    {round.note && <p className="mt-1 line-clamp-2 text-sm text-muted">{round.note}</p>}
                  </div>
                  <div className="flex items-center gap-1">
                    <ViewRoundLink roundId={round.id} roundName={round.name} />
                ...
                <div className="mt-4 flex items-center justify-between border-t border-border pt-3">
                  <div>
                    ...
                    <p className="mt-0.5 font-semibold">{formatCurrencyTHB(round.totalSatang)}</p>
                  </div>
                  <div className="text-right">
                    <span className={`inline-flex rounded px-2 py-0.5 text-xs font-medium ${ROUND_STATUS_BADGE_CLASSES[status]}`}>{ROUND_STATUS_LABELS[status]}</span>
                    <p className="mt-1 text-xs text-muted">{round.itemCount} รายการ · {new Date(round.createdAt).toLocaleDateString("th-TH")}</p>
                  </div>
                </div>
              </li>
            ))}
            ...
            <table className="w-full min-w-220 border-collapse text-xs">
              <thead>
                <tr className="border-b border-border bg-muted-surface text-left text-muted">
                  ...
                  <th className="w-24 px-4 py-3"><span className="sr-only">จัดการ</span></th>
                </tr>
              </thead>
              <tbody>
                {rounds.map((round) => (
                  <tr key={round.id} className="border-b border-border last:border-0 hover:bg-muted-surface/70">
                    ...
                    <td className="px-4 py-1.5"><span className={`inline-flex rounded px-2 py-0.5 text-xs font-medium ${ROUND_STATUS_BADGE_CLASSES[status]}`}>{ROUND_STATUS_LABELS[status]}</span></td>
...
function ViewRoundLink({ roundId, roundName }: { roundId: number; roundName: string }) {
  return (
    <Link
      href={`/rounds/${roundId}`}
      aria-label={`ดูรายละเอียดรอบ ${roundName}`}
      className="inline-flex items-center gap-1 rounded-lg p-2 text-muted transition-colors hover:bg-muted-surface hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring/30 focus-visible:outline-none"
    >
      <IconEye aria-hidden className="h-4 w-4" />
    </Link>
  );
}
